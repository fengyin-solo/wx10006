<template>
  <section class="page" data-module="dispatch">
    <header class="page-head">
      <div>
        <h2>调度指令管理</h2>
        <p class="page-desc">维护调度指令，围绕指令编号、指令类型、执行班组、下发时间做登记、筛选与状态流转。</p>
      </div>
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr v-if="!rows.length">
          <td :colspan="columns.length + 1" class="empty-state">暂无调度指令数据，可先登记调度指令</td>
        </tr>
      </tbody>
    </table>
...
const columns = ["指令编号", "指令类型", "执行班组", "下发时间", "截止时间", "完成时间", "指令内容", "指令状态"]
const actions = ["确认下达", "开始执行", "确认完成"]
const statuses = ["待下达", "已下达", "执行中", "已完成", "已超时"]
const stats = [{"label": "待下达指令", "value": 0}, {"label": "执行中指令", "value": 0}, {"label": "超时指令", "value": 0}]
